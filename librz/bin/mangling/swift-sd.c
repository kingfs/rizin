...
static char *swift_demangle_cmd(const char *s) {
	...
	if (have_swift_demangle == -1) {
		if (!swift_demangle) {
			...
			if (!swift_demangle || !strcmp(swift_demangle, "swift-demangle")) {
				char *xcrun = rz_file_path("xcrun");
				if (xcrun) {
					if (strcmp(xcrun, "xcrun")) {
						free(swift_demangle);
						swift_demangle = rz_str_newf("%s swift-demangle", xcrun);
	...
	if (swift_demangle) {
		...
		//char *res = rz_sys_cmd_strf ("%s -compact -simplified '%s'",
		char *res = rz_sys_cmd_strf("%s -compact '%s'",
			swift_demangle, s);
		if (res && !*res) {
			free(res);
			res = NULL;
...
static char *swift_demangle_lib(const char *s) {
	...
	if (!haveSwiftCore) {
		void *lib = rz_lib_dl_open("/usr/lib/swift/libswiftCore.dylib");
		if (lib) {
			swift_demangle = rz_lib_dl_sym(lib, "swift_demangle");
		}
		haveSwiftCore = true;
	}
	if (swift_demangle) {
