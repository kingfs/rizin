...
int cplus_demangle_fill_name(struct demangle_component *p, const char *s, int len) {
...
CP_STATIC_IF_GLIBCPP_V3
int cplus_demangle_fill_ctor(struct demangle_component *p,
	enum gnu_v3_ctor_kinds kind,
	struct demangle_component *name) {
	if (p == NULL || name == NULL || (int)kind < gnu_v3_complete_object_ctor || (int)kind > gnu_v3_object_ctor_group)
		return 0;
	p->d_printing = 0;
...
CP_STATIC_IF_GLIBCPP_V3
int cplus_demangle_fill_dtor(struct demangle_component *p,
	enum gnu_v3_dtor_kinds kind,
	struct demangle_component *name) {
	if (p == NULL || name == NULL || (int)kind < gnu_v3_deleting_dtor || (int)kind > gnu_v3_object_dtor_group)
		return 0;
	p->d_printing = 0;
...
cplus_demangle_mangled_name(struct d_info *di, int top_level) {
	...
	if (top_level && (di->options & DMGL_PARAMS) != 0)
		while (d_peek_char(di) == '.' && (IS_LOWER(d_peek_next_char(di)) || d_peek_next_char(di) == '_' || IS_DIGIT(d_peek_next_char(di))))
	     ...
	     which is local to a function.  */
			...
			if (peek != '\0' && peek != 'E') {
		     ...
		     this is nested within.  */
					if (!top_level && dc->type == DEMANGLE_COMPONENT_LOCAL_NAME && ftype->type == DEMANGLE_COMPONENT_FUNCTION_TYPE)
						d_left(ftype) = NULL;
	 ...
	 <template-param> here.  */
		...
		if (peek == 'D') {
			...
			if (peek2 == 'T' || peek2 == 't')
				...
				/* Destructor name.  */
				dc = d_unqualified_name(di);
		} else if (IS_DIGIT(peek) || IS_LOWER(peek) || peek == 'C' || peek == 'U' || peek == 'L')
			dc = d_unqualified_name(di);
		else if (peek == 'S')
...
d_identifier(struct d_info *di, int len) {
	...
	/* A Java mangled name may have a trailing '$' if it is a C++
     ...
     ignore the '$'.  */
	if ((di->options & DMGL_JAVA) != 0 && d_peek_char(di) == '$')
		d_advance(di, 1);

	/* Look for something which looks like a gcc encoding of an
     anonymous namespace, and replace it with a more user friendly
     name.  */
	if (len >= (int)ANONYMOUS_NAMESPACE_PREFIX_LEN + 2 && memcmp(name, ANONYMOUS_NAMESPACE_PREFIX, ANONYMOUS_NAMESPACE_PREFIX_LEN) == 0) {
		const char *s;
...
d_operator_name(struct d_info *di) {
	...
	if (c1 == 'v' && IS_DIGIT(c2))
	 ...
	 the sentinel at the end of the array.  */
		int high = ((sizeof(cplus_demangle_operators) / sizeof(cplus_demangle_operators[0])) - 1);
...
const struct demangle_builtin_type_info
	cplus_demangle_builtin_types[D_BUILTIN_TYPE_COUNT] = {
		...
		/* y */ { NL("unsigned long long"), NL("unsigned long long"), D_PRINT_UNSIGNED_LONG_LONG },
		...
		/* 32 */ { NL("decltype(nullptr)"), NL("decltype(nullptr)"), D_PRINT_DEFAULT },
	};
...
cplus_demangle_type(struct d_info *di) {
	...
	if (next_is_type_qual(di)) {
		...
		if (!*pret)
			return NULL;
		if ((*pret)->type == DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS || (*pret)->type == DEMANGLE_COMPONENT_REFERENCE_THIS) {
...
d_parmlist(struct d_info *di) {
	...
	if (d_right(tl) == NULL && d_left(tl)->type == DEMANGLE_COMPONENT_BUILTIN_TYPE && d_left(tl)->u.s_builtin.type->print == D_PRINT_VOID) {
...
d_template_args(struct d_info *di) {
	if (d_peek_char(di) != 'I' && d_peek_char(di) != 'J')
		return NULL;
	d_advance(di, 1);
...
op_is_new_cast(struct demangle_component *op) {
	...
	return (code[1] == 'c' && (code[0] == 's' || code[0] == 'd' || code[0] == 'c' || code[0] == 'r'));
}
...
d_expression_1(struct d_info *di) {
	...
	if (peek == 'L')
	...
	} else if (IS_DIGIT(peek) || (peek == 'o' && d_peek_next_char(di) == 'n')) {
		...
		if (d_peek_char(di) == 'I')
			...
			return name;
	} else if ((peek == 'i' || peek == 't') && d_peek_next_char(di) == 'l') {
		...
		case 1: {
			...
			if (code && (code[0] == 'p' || code[0] == 'm') && code[1] == code[0])
				/* pp_ and mm_ are the prefix variants.  */
				suffix = !d_check_char(di, '_');

			if (op->type == DEMANGLE_COMPONENT_CAST && d_check_char(di, '_'))
		...
		case 3: {
			...
			if (code == NULL)
				...
				if (d_peek_char(di) == 'E') {
					d_advance(di, 1);
					third = NULL;
				} else if (d_peek_char(di) == 'p' && d_peek_next_char(di) == 'i') {
					/* Parenthesized initializer.  */
					d_advance(di, 2);
					third = d_exprlist(di, 'E');
				} else if (d_peek_char(di) == 'i' && d_peek_next_char(di) == 'l')
					/* initializer-list.  */
					third = d_expression_1(di);
				else
					return NULL;
...
d_expr_primary(struct d_info *di) {
	...
	if (d_peek_char(di) == '_'
	 ...
	 print the type name itself.  */
		if (type->type == DEMANGLE_COMPONENT_BUILTIN_TYPE && type->u.s_builtin.type->print != D_PRINT_DEFAULT)
...
d_local_name(struct d_info *di) {
	...
	if (d_peek_char(di) == 's') {
		...
		if (name
	     ...
	     and are not functions.  */
			&& name->type != DEMANGLE_COMPONENT_LAMBDA && name->type != DEMANGLE_COMPONENT_UNNAMED_TYPE) {
	...
	if (function->type == DEMANGLE_COMPONENT_TYPED_NAME && d_right(function)->type == DEMANGLE_COMPONENT_FUNCTION_TYPE)
...
#else
		dpi.saved_scopes = alloca(dpi.num_saved_scopes * sizeof(*dpi.saved_scopes));
		dpi.copy_templates = alloca(dpi.num_copy_templates * sizeof(*dpi.copy_templates));
#endif
...
d_pack_length(const struct demangle_component *dc) {
	int count = 0;
	while (dc && dc->type == DEMANGLE_COMPONENT_TEMPLATE_ARGLIST && d_left(dc) != NULL) {
		++count;
		dc = d_right(dc);
	}
	return count;
...
d_print_subexpr(struct d_print_info *dpi, int options,
	struct demangle_component *dc) {
	int simple = 0;
	if (dc && (dc->type == DEMANGLE_COMPONENT_NAME || dc->type == DEMANGLE_COMPONENT_QUAL_NAME || dc->type == DEMANGLE_COMPONENT_INITIALIZER_LIST || dc->type == DEMANGLE_COMPONENT_FUNCTION_PARAM))
		simple = 1;
	if (!simple)
		d_append_char(dpi, '(');
	d_print_comp(dpi, options, dc);
...
d_print_comp_inner(struct d_print_info *dpi, int options,
	...
	case DEMANGLE_COMPONENT_TEMPLATE: {
	   ...
	   Instead, treat the template essentially as a name.  */
		...
		if ((options & DMGL_JAVA) != 0 && dcl->type == DEMANGLE_COMPONENT_NAME && dcl->u.s_name.len == 6 && strncmp(dcl->u.s_name.s, "JArray", 6) == 0) {
	...
	case DEMANGLE_COMPONENT_CONST: {
	   ...
	   We only need to print it once.  */
		...
		for (pdpm = dpi->modifiers; pdpm != NULL; pdpm = pdpm->next) {
			if (!pdpm->printed) {
				if (pdpm->mod->type != DEMANGLE_COMPONENT_RESTRICT && pdpm->mod->type != DEMANGLE_COMPONENT_VOLATILE && pdpm->mod->type != DEMANGLE_COMPONENT_CONST)
					break;
	...
	case DEMANGLE_COMPONENT_RVALUE_REFERENCE: {
		...
		if (!dpi->is_lambda_arg && sub->type == DEMANGLE_COMPONENT_TEMPLATE_PARAM) {
		   ...
		   need to restore SUB's template stack temporarily.  */
				for (dcse = dpi->component_stack; dcse != NULL;
					dcse = dcse->parent) {
					if (dcse->dc == sub || (dcse->dc == dc && dcse != dpi->component_stack)) {
		...
		if (sub->type == DEMANGLE_COMPONENT_REFERENCE || sub->type == dc->type)
			dc = sub;
	...
	case DEMANGLE_COMPONENT_ARRAY_TYPE: {
	   ...
	   stack frame after we return.  */
		...
		i = 1;
		pdpm = hold_modifiers;
		while (pdpm != NULL && (pdpm->mod->type == DEMANGLE_COMPONENT_RESTRICT || pdpm->mod->type == DEMANGLE_COMPONENT_VOLATILE || pdpm->mod->type == DEMANGLE_COMPONENT_CONST)) {
			if (!pdpm->printed) {
				if (i >= sizeof adpm / sizeof adpm[0]) {
					d_print_error(dpi);
					return;
	...
	case DEMANGLE_COMPONENT_FIXED_TYPE:
		...
		if (dc->u.s_fixed.length->u.s_builtin.type != &cplus_demangle_builtin_types['i' - 'a']) {
	...
	case DEMANGLE_COMPONENT_UNARY: {
		...
		if (op->type == DEMANGLE_COMPONENT_OPERATOR) {
		   ...
		   function.  */
				if (operand->type == DEMANGLE_COMPONENT_TYPED_NAME && d_left(operand)->type == DEMANGLE_COMPONENT_QUAL_NAME && d_right(operand)->type == DEMANGLE_COMPONENT_FUNCTION_TYPE)
	...
	case DEMANGLE_COMPONENT_BINARY:
	 ...
	 with the '>' which ends the template parameters.  */
		if (d_left(dc)->type == DEMANGLE_COMPONENT_OPERATOR && d_left(dc)->u.s_operator.op->len == 1 && d_left(dc)->u.s_operator.op->name[0] == '>')
			d_append_char(dpi, '(');

		if (strcmp(d_left(dc)->u.s_operator.op->code, "cl") == 0 && d_left(d_right(dc))->type == DEMANGLE_COMPONENT_TYPED_NAME) {
	...
	case DEMANGLE_COMPONENT_LITERAL_NEG: {
		...
		if (d_left(dc)->type == DEMANGLE_COMPONENT_BUILTIN_TYPE) {
			...
			case D_PRINT_BOOL:
				if (d_right(dc)->type == DEMANGLE_COMPONENT_NAME && d_right(dc)->u.s_name.len == 1 && dc->type == DEMANGLE_COMPONENT_LITERAL) {
...
d_print_mod_list(struct d_print_info *dpi, int options,
	...
	if (mods->printed || (!suffix && (is_fnqual_component_type(mods->mod->type)))) {
...
CP_STATIC_IF_GLIBCPP_V3
void cplus_demangle_init_info(const char *mangled, int options, size_t len,
	struct d_info *di) {
	di->s = mangled;
	di->send = mangled + len;
	di->options = options;
...
d_demangle_callback(const char *mangled, int options,
	...
	if (mangled[0] == '_' && mangled[1] == 'Z')
		type = DCT_MANGLED;
	else if (strncmp(mangled, "_GLOBAL_", 8) == 0 && (mangled[8] == '.' || mangled[8] == '_' || mangled[8] == '$') && (mangled[9] == 'D' || mangled[9] == 'I') && mangled[10] == '_')
