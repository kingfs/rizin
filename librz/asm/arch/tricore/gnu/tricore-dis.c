...
#define MATCHES_ISA(isa) \
	(((isa) == TRICORE_GENERIC) || (((isa)&bfd_mach_rider_mask) & current_isa))
...
static const char *
	find_core_reg(addr) unsigned long addr;
{
	struct sfrlist *psfr;
	int idx = addr & 0xff;
...
#define DFILE info->stream
	...
	if (((*insn->name == 'n') && !strcmp(insn->name, "nor")) && ((insn->nr_operands == 1) || ((insn->nr_operands == 3) && (insn->args[2] == 'n') && (dec_insn.regs[0] == dec_insn.regs[1]) && (dec_insn.cexp[2] == 0)))) {
	...
	} else if (expect_lea && (!strcmp(insn->name, "lea") || !strncmp(insn->name, "ld.", 3) || !strncmp(insn->name, "st.", 3) || !strncmp(insn->name, "swap", 4) || !strcmp(insn->name, "ldmst"))) {
		if (insn->nr_operands == 3) {
			if ((!strcmp(insn->name, "lea") || !strncmp(insn->name, "ld.", 3) || !strcmp(insn->name, "ldmst"))) {
	...
	} else if (!strncmp(insn->name, "ret", 3)) {
		int i = 0;
		for (i = 2; i < 8; i++) {
			load_areg[i] = false;
		}
