...
#define MASK_V9 ((1 << SPARC_OPCODE_ARCH_V9) | (1 << SPARC_OPCODE_ARCH_V9A) | (1 << SPARC_OPCODE_ARCH_V9B))
...
static char *reg_names[] = { "g0", "g1", "g2", "g3", "g4", "g5", "g6", "g7",
...
compute_arch_mask(unsigned long mach) {
	...
	case bfd_mach_sparc_sparclite_le:
	 ...
	 indicating generic v8 is also selected.  */
		return (SPARC_OPCODE_ARCH_MASK(SPARC_OPCODE_ARCH_SPARCLITE) | SPARC_OPCODE_ARCH_MASK(SPARC_OPCODE_ARCH_V8));
...
int print_insn_sparc(bfd_vma memaddr, disassemble_info *info) {
	...
	if (!opcodes_initialized || info->mach != current_mach) {
		int i;
...
#undef creg
					...
					case 'h':
						(*info->fprintf_func)(stream, "%%hi(%#x)",
							((unsigned)0xFFFFFFFF & ((int)X_IMM22(insn) << 10)));
						break;
			 ...
			 not before it.  */
					...
					case 'n':
						(*info->fprintf_func)(stream, "%#x", SIGN_EXT(X_DISP22(insn), 22));
						break;
		 ...
		 the previous instruction was not sethi.  */
				if (errcode == 0) {
					...
					if ((prev_insn & 0xc1c00000) == 0x01000000 && X_RD(prev_insn) == X_RS1(insn)) {
						(*info->fprintf_func)(stream, "\t! ");
						info->target =
							((unsigned)0xFFFFFFFF & ((int)X_IMM22(prev_insn) << 10));
						if (imm_added_to_rs1) {
