...
#define ARM_AEXT_NOTM \
	(ARM_AEXT_V4 | ARM_EXT_V5ExP | ARM_EXT_V5J | ARM_EXT_V6_NOTM | ARM_EXT_V6_DSP)
...
#define ARM_AEXT_V7M \
	((ARM_AEXT_V7_ARM | ARM_EXT_V6M | ARM_EXT_V7M | ARM_EXT_DIV) & ~(ARM_AEXT_NOTM))
#define ARM_AEXT_V7 (ARM_AEXT_V7A & ARM_AEXT_V7R & ARM_AEXT_V7M)
#define ARM_AEXT_V7EM \
	(ARM_AEXT_V7M | ARM_EXT_V5ExP | ARM_EXT_V6_DSP)
#define ARM_AEXT_V8A \
	(ARM_AEXT_V7A | ARM_EXT_MP | ARM_EXT_SEC | ARM_EXT_DIV | ARM_EXT_ADIV | ARM_EXT_VIRT | ARM_EXT_V8)
...
#define ARM_ARCH_IWMMXT2 \
	ARM_FEATURE_LOW(ARM_AEXT_V5TE, ARM_CEXT_XSCALE | ARM_CEXT_IWMMXT | ARM_CEXT_IWMMXT2)
...
#define FPU_ARCH_NEON_VFP_ARMV8_1 \
	ARM_FEATURE_COPROC(FPU_NEON_ARMV8 | FPU_VFP_ARMV8 | FPU_NEON_EXT_RDMA)
#define FPU_ARCH_CRYPTO_NEON_VFP_ARMV8_1 \
	ARM_FEATURE_COPROC(FPU_CRYPTO_ARMV8 | FPU_NEON_ARMV8 | FPU_VFP_ARMV8 | FPU_NEON_EXT_RDMA)
...
/* Extensions containing some Thumb-2 instructions.  If any is present, Thumb
   ISA is Thumb-2.  */
#define ARM_ARCH_THUMB2 ARM_FEATURE_CORE(ARM_EXT_V6T2 | ARM_EXT_V7 | ARM_EXT_DIV | ARM_EXT_V8, \
...
#define ARM_CPU_HAS_FEATURE(CPU, FEAT) \
	(((CPU).core[0] & (FEAT).core[0]) != 0 || ((CPU).core[1] & (FEAT).core[1]) != 0 || ((CPU).coproc & (FEAT).coproc) != 0)

/* Tests whether the features of A are a subset of B.  */
#define ARM_FSET_CPU_SUBSET(A, B) \
	(((A).core[0] & (B).core[0]) == (A).core[0] && ((A).core[1] & (B).core[1]) == (A).core[1] && ((A).coproc & (B).coproc) == (A).coproc)

#define ARM_CPU_IS_ANY(CPU) \
	((CPU).core[0] == ((arm_feature_set)ARM_ANY).core[0] && (CPU).core[1] == ((arm_feature_set)ARM_ANY).core[1])
...
#define ARM_FEATURE_EQUAL(T1, T2) \
	((T1).core[0] == (T2).core[0] && (T1).core[1] == (T2).core[1] && (T1).coproc == (T2).coproc)
...
#define ARM_FEATURE_LOW(core, coproc) \
	{ { (core), 0 }, (coproc) }
#define ARM_FEATURE_CORE(core1, core2) \
	{ { (core1), (core2) }, 0 }
#define ARM_FEATURE_CORE_LOW(core) \
	{ { (core), 0 }, 0 }
#define ARM_FEATURE_CORE_HIGH(core) \
	{ { 0, (core) }, 0 }
#define ARM_FEATURE_COPROC(coproc) \
	{ { 0, 0 }, (coproc) }
#define ARM_FEATURE(core1, core2, coproc) \
	{ { (core1), (core2) }, (coproc) }
