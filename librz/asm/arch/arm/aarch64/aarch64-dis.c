...
get_greg_qualifier_from_value(aarch64_insn value) {
	...
	assert(value <= 0x1 && aarch64_get_qualifier_standard_value(qualifier) == value);
	return qualifier;
}
...
get_vreg_qualifier_from_value(aarch64_insn value) {
	...
	assert(value <= 0x8 && aarch64_get_qualifier_standard_value(qualifier) == value);
	return qualifier;
}
...
get_sreg_qualifier_from_value(aarch64_insn value) {
	...
	assert(value <= 0x4 && aarch64_get_qualifier_standard_value(qualifier) == value);
	return qualifier;
}
...
aarch64_ext_regrt_sysins(const aarch64_operand *self, aarch64_opnd_info *info,
	...
	aarch64_operand_error *errors ATTRIBUTE_UNUSED) {
	info->reg.regno = extract_field(self->fields[0], code, 0);
	assert(info->idx == 1 && (aarch64_get_operand_class(inst->operands[0].type) == AARCH64_OPND_CLASS_SYSTEM));
...
aarch64_ext_reglane(const aarch64_operand *self, aarch64_opnd_info *info,
	...
	if (inst->opcode->iclass == asisdone || inst->opcode->iclass == asimdins) {
		if (info->type == AARCH64_OPND_En && inst->opcode->operands[0] == AARCH64_OPND_Ed) {
			unsigned shift;
		...
		if (inst->opcode->op == OP_FCMLA_ELEM && info->qualifier != AARCH64_OPND_QLF_S_H) {
...
aarch64_ext_advsimd_imm_shift(const aarch64_operand *self ATTRIBUTE_UNUSED,
	...
	assert((iclass == asimdshf || iclass == asisdshf) && (info->type == AARCH64_OPND_IMM_VLSR || info->type == AARCH64_OPND_IMM_VLSL));
...
decode_limm(uint32_t esize, aarch64_insn value, int64_t *result) {
	...
	if (N != 0) {
		...
		if (S >= 0x00 && S <= 0x1f) {
			...
			simd_size = 16;
			S &= 0xf;
		} else if (S >= 0x30 && S <= 0x37) {
			simd_size = 8;
			S &= 0x7;
		} else if (S >= 0x38 && S <= 0x3b) {
			simd_size = 4;
			S &= 0x3;
		} else if (S >= 0x3c && S <= 0x3d) {
			simd_size = 2;
			S &= 0x1;
		} else {
			return 0;
		...
		case 0x20 ... 0x2f: /* 10xxxx */
			simd_size = 16;
			S &= 0xf;
			break;
		case 0x30 ... 0x37: /* 110xxx */
			simd_size = 8;
			S &= 0x7;
			break;
		case 0x38 ... 0x3b: /* 1110xx */
			simd_size = 4;
			S &= 0x3;
			break;
		case 0x3c ... 0x3d: /* 11110x */
			simd_size = 2;
			S &= 0x1;
			break;
		default: return FALSE;
...
aarch64_ext_ft(const aarch64_operand *self ATTRIBUTE_UNUSED,
	...
	if (inst->opcode->iclass == ldstpair_indexed || inst->opcode->iclass == ldstnapair_offs || inst->opcode->iclass == ldstpair_off || inst->opcode->iclass == loadlit) {
...
aarch64_ext_sysreg(const aarch64_operand *self ATTRIBUTE_UNUSED,
	...
	if (inst->opcode->iclass == ic_system) {
	 ...
	 if it's both or unspecified don't care.  */
		...
		else if ((inst->opcode->flags & (F_SYS_READ | F_SYS_WRITE)) == F_SYS_WRITE)
...
aarch64_ext_sve_addr_zz(const aarch64_operand *self, aarch64_opnd_info *info,
	...
	info->shifter.operator_present = (kind != AARCH64_MOD_LSL || info->shifter.amount != 0);
	info->shifter.amount_present = (info->shifter.amount != 0);
	return TRUE;
}
...
aarch64_ext_sve_aimm(const aarch64_operand *self,
	...
	const aarch64_inst *inst,
	aarch64_operand_error *errors) {
	return (aarch64_ext_imm(self, info, code, inst, errors) && decode_sve_aimm(info, (uint8_t)info->imm.value));
}
...
aarch64_ext_sve_asimm(const aarch64_operand *self,
	...
	const aarch64_inst *inst,
	aarch64_operand_error *errors) {
	return (aarch64_ext_imm(self, info, code, inst, errors) && decode_sve_aimm(info, (int8_t)info->imm.value));
}
...
aarch64_ext_sve_limm_mov(const aarch64_operand *self,
	...
	return (aarch64_ext_limm(self, info, code, inst, errors) && aarch64_sve_dupm_mov_immediate_p(info->imm.value, esize));
}
...
aarch64_ext_sve_shlimm(const aarch64_operand *self,
	aarch64_opnd_info *info, const aarch64_insn code,
	const aarch64_inst *inst, aarch64_operand_error *errors) {
	if (!aarch64_ext_imm(self, info, code, inst, errors) || info->imm.value == 0)
		return FALSE;
...
decode_sizeq(aarch64_inst *inst) {
	...
	if (debug_dump) {
		int i;
		for (i = 0; candidates[i] != AARCH64_OPND_QLF_NIL && i < AARCH64_MAX_QLF_SEQ_NUM; i++)
...
do_misc_decoding(aarch64_inst *inst) {
	...
	case OP_MOVS_P_P:
		...
		return (value == extract_field(FLD_SVE_Pm, inst->value, 0) && value == extract_field(FLD_SVE_Pg4_10, inst->value, 0));

	case OP_MOV_Z_P_Z:
		return (extract_field(FLD_SVE_Zd, inst->value, 0) == extract_field(FLD_SVE_Zm_16, inst->value, 0));
	...
	case OP_MOV_Z_Z:
		return (extract_field(FLD_SVE_Zn, inst->value, 0) == extract_field(FLD_SVE_Zm_16, inst->value, 0));
	...
	case OP_MOVM_P_P_P:
		return (extract_field(FLD_SVE_Pd, inst->value, 0) == extract_field(FLD_SVE_Pm, inst->value, 0));

	case OP_MOVZS_P_P_P:
	case OP_MOVZ_P_P_P:
		return (extract_field(FLD_SVE_Pn, inst->value, 0) == extract_field(FLD_SVE_Pm, inst->value, 0));

	case OP_NOTS_P_P_P_Z:
	case OP_NOT_P_P_P_Z:
		return (extract_field(FLD_SVE_Pm, inst->value, 0) == extract_field(FLD_SVE_Pg4_10, inst->value, 0));
...
do_special_decoding(aarch64_inst *inst) {
	...
	if (inst->opcode->flags & F_T) {
		...
		int num = 0;
		unsigned val, Q;
		assert(aarch64_get_operand_class(inst->opcode->operands[0]) == AARCH64_OPND_CLASS_SIMD_REG);
		/* imm5<3:0>	q	<t>
	 0000		x	reserved
	 xxx1		0	8b
	 xxx1		1	16b
	...
	if (inst->opcode->flags & F_GPRSIZE_IN_Q) {
	 ...
	 STXP <Ws>, <Xt1>, <Xt2>, [<Xn|SP>{,#0}].  */
		...
		if (idx == -1) {
	     ...
	     register.  */
			assert(aarch64_get_operand_class(inst->opcode->operands[0]) == AARCH64_OPND_CLASS_INT_REG);
			idx = 0;
		}
		assert(idx == 0 || idx == 1);
	...
	if (inst->opcode->flags & F_LDS_SIZE) {
		...
		value = extract_field_2(&field, inst->value, 0);
		inst->operands[0].qualifier = value ? AARCH64_OPND_QLF_W : AARCH64_OPND_QLF_X;
	}
...
convert_ubfm_to_lsl(aarch64_inst *inst) {
	...
	int64_t val = inst->operands[2].qualifier == AARCH64_OPND_QLF_imm_0_31 ? 31 : 63;
...
convert_from_csel(aarch64_inst *inst) {
	if (inst->operands[1].reg.regno == inst->operands[2].reg.regno && (inst->operands[3].cond->value & 0xe) != 0xe) {
		copy_operand_info(inst, 2, 3);
...
convert_csinc_to_cset(aarch64_inst *inst) {
	if (inst->operands[1].reg.regno == 0x1f && inst->operands[2].reg.regno == 0x1f && (inst->operands[3].cond->value & 0xe) != 0xe) {
...
convert_movebitmask_to_mov(aarch64_inst *inst) {
	...
	/* ORR has an immediate that could be generated by a MOVZ or MOVN
     instruction.  */
	if (inst->operands[0].reg.regno != 0x1f && (aarch64_wide_constant_p(value, is32, NULL) || aarch64_wide_constant_p(~value, is32, NULL)))
		return 0;
...
aarch64_opcode_decode(const aarch64_opcode *opcode, const aarch64_insn code,
	...
	for (i = 0; i < AARCH64_MAX_OPND_NUM; i++) {
		...
		if (operand_has_extractor(opnd) && (!aarch64_extract_operand(opnd, &inst->operands[i], code, inst, errors))) {
...
int aarch64_decode_insn(aarch64_insn insn, aarch64_inst *inst,
...
get_sym_code_type(struct disassemble_info *info, int n,
	...
	if (name[0] == '$' && (name[1] == 'x' || name[1] == 'd') && (name[2] == '\0' || name[2] == '.')) {
