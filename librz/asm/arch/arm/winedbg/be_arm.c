...
static ut32 arm_disasm_branchreg(struct winedbg_arm_insn *arminsn, ut32 inst) {
	arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "b%s %s", get_cond(inst), tbl_regs[get_nibble(inst, 0)]);
	return 0;
}
...
static ut32 arm_disasm_singletrans(struct winedbg_arm_insn *arminsn, ut32 inst) {
	...
	if (indexing) {
		if (immediate) {
			arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "[%s, #%d]", tbl_regs[get_nibble(inst, 4)], offset);
		} else if (((inst >> 4) & 0xff) == 0x00) { /* no shift */
			arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "[%s, %s]", tbl_regs[get_nibble(inst, 4)],
				tbl_regs[get_nibble(inst, 0)]);
...
static ut32 arm_disasm_blocktrans(struct winedbg_arm_insn *arminsn, ut32 inst) {
	...
	for (i = 0; i <= 15; i++) {
		if ((inst >> i) & 1) {
			if (i == last) {
				arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "%s", tbl_regs[i]);
			} else {
				arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "%s, ", tbl_regs[i]);
			}
		}
...
static ut32 arm_disasm_coprocdatatrans(struct winedbg_arm_insn *arminsn, ut32 inst) {
	...
	if (indexing) {
		arminsn->str_asm = rz_str_appendf(arminsn->str_asm, " %u, cr%u, [%s, #%d]%s", CPnum, CRd, tbl_regs[get_nibble(inst, 4)],
			offset, writeback ? "!" : "");
	} else {
		arminsn->str_asm = rz_str_appendf(arminsn->str_asm, " %u, cr%u, [%s], #%d", CPnum, CRd, tbl_regs[get_nibble(inst, 4)],
			offset);
	}
	return 0;
}
...
static ut16 thumb_disasm_pushpop(struct winedbg_arm_insn *arminsn, ut16 inst) {
	...
	if (lrpc) {
		arminsn->str_asm = rz_str_appendf(arminsn->str_asm, "%s%s", last ? ", " : "", load ? "pc" : "lr");
	}
