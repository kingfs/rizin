...
xtensa_isa_init(xtensa_isa_status *errno_p, char **error_msg_p) {
	...
	for (is_user = 0; is_user < 2; is_user++) {
		isa->sysreg_table[is_user] =
			bfd_malloc((isa->max_sysreg_num[is_user] + 1) * sizeof(xtensa_sysreg));
...
int xtensa_operand_encode(xtensa_isa isa, xtensa_opcode opc, int opnd,
	...
	/* Encode the value.  In some cases, the encoding function may detect
     ...
     the original value.  */
	orig_val = *valp;
	if ((*intop->encode)(valp) || (test_val = *valp, (*intop->decode)(&test_val)) || test_val != orig_val) {
	...
	do {                                                                                \
		if ((IFOP) < 0 || (IFOP) >= (ICLASS)->num_interfaceOperands) {              \
			...
			sprintf(xtisa_error_msg, "invalid interface operand number (%d); "  \
						 "opcode \"%s\" has %d interface operands", \
				(IFOP),                                                     \
...
xtensa_sysreg_lookup(xtensa_isa isa, int num, int is_user) {
	...
	if (num < 0 || num > intisa->max_sysreg_num[is_user] || intisa->sysreg_table[is_user][num] == XTENSA_UNDEFINED) {
		xtisa_errno = xtensa_isa_bad_sysreg;
