	 ...
	 value for later handling by either %S (shimm) or %L (limm).  */
		...
		if ((reg->type == AUXREG) || (reg->type == AUXREG_AC)) {
		 ...
		 would be stored in this fashion.  */
			...
			if (!arc_mach_a4 && (('B' == operand->fmt) || ('#' == operand->fmt) || ('g' == operand->fmt) || ('(' == operand->fmt) || ('{' == operand->fmt) || ('<' == operand->fmt))) {
...
insert_offset(arc_insn insn, long *ex ATTRIBUTE_UNUSED,
	...
	if (reg != NULL) {
		if (arc_mach_a4) {
			arc_insn myinsn = (insert_reg(0, ex, operand, mods, reg, value, errmsg) >> operand->shift);
...
#define LD_SYNTAX(D, B, O) \
	((ls_operand[LS_DEST] == (D) && ls_operand[LS_BASE] == (B) && ls_operand[LS_OFFSET] == (O)))
...
insert_reladdr(arc_insn insn, long *ex ATTRIBUTE_UNUSED,
	...
	} else if (!arc_mach_a4 && (('s' == operand->fmt) || ('S' == operand->fmt) || ('Z' == operand->fmt))) {
		if (value & 1) {
			*errmsg = _("branch address not on 2 byte boundary");
		}
...
extract_reg(arc_insn *insn,
	...
	if ((mods & ARC_MOD_AUXREG) && ARC_REG_CONSTANT_P(regno)) {
...
static const struct arc_operand_value arc_reg_names_a500600[] = {
	...
	{ "r0", 0, REG_AC, ARC_REGISTER_16 },
	{ "r1", 1, REG_AC, ARC_REGISTER_16 },
	{ "r2", 2, REG_AC, ARC_REGISTER_16 },
	{ "r3", 3, REG_AC, ARC_REGISTER_16 },
	{ "r12", 4, REG_AC, ARC_REGISTER_16 },
	{ "r13", 5, REG_AC, ARC_REGISTER_16 },
	{ "r14", 6, REG_AC, ARC_REGISTER_16 },
	{ "r15", 7, REG_AC, ARC_REGISTER_16 },
...
int arc_insn_not_jl(arc_insn insn) {
	return ((insn & (I(-1) | A(-1) | C(-1) | R(-1, 7, 1) | R(-1, 9, 1))) != (I(0x7) | R(-1, 9, 1)));
}
...
*/
int ac_get_load_sdasym_insn_type(arc_insn insn, int compact_insn_16) {
	int load_type = -1;
