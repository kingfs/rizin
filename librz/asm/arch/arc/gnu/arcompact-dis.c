 ...
 */
static bfd_vma
	bfd_getm32(data) unsigned int data;
{
	bfd_vma value = 0;
 ...
 */
static bfd_vma
	bfd_getm32_ac(data) unsigned int data;
{
	bfd_vma value = 0;
...
dsmOneArcInst(bfd_vma addr, struct arcDisState *state, disassemble_info *info) {
   ...
   * instead of all over this crazy switch case. */
    ...
    */
		...
		write_instr_name();
		(decodingClass == 37 ? WRITE_FORMAT_x_COMMA_LB(A) : WRITE_FORMAT_x_COMMA(A));
		WRITE_FORMAT_x_COMMA(B);
		(decodingClass == 37 ? WRITE_FORMAT_x_RB(C) : WRITE_FORMAT_x(C));
		WRITE_NOP_COMMENT();
		my_sprintf(state, state->operandBuffer, formatString, fieldA, fieldB, fieldC);
 ...
 */
static const char *
_auxRegName(void *_this ATTRIBUTE_UNUSED, /* C++ this pointer */
	int v /* Register value */
) {
