		...
		rz_list_foreach(list, iter, map) {
			if ((!libname ||
				...
				if (sectname) {
					...
					res = rz_sys_cmd_strf("env RZ_BIN_PREFIX=\"%s\" rz_bin %s-B 0x%08" PFMT64x " -S \"%s\" | grep \"%s\"", name, mode, baddr, filesc, sect);
					free(sect);
				} else {
					res = rz_sys_cmd_strf("env RZ_BIN_PREFIX=\"%s\" rz_bin %s-B 0x%08" PFMT64x " -S \"%s\"", name, mode, baddr, filesc);
				}
				free(filesc);
				rz_cons_println(res);
				free(name);
...
static int showreg(RzCore *core, const char *str, bool use_color) {
	...
	if (bits) {
		rz_debug_reg_list (core->dbg, RZ_REG_TYPE_GPR, bits, str[0], use_color? Color_GREEN: NULL);
	} else {
...
static void get_backtrace_info(RzCore *core, RzDebugFrame *frame, ut64 addr, char **flagdesc, char **flagdesc2, char **pcstr, char **spstr, bool hex_format) {
	...
	*flagdesc2 = NULL;
	...
	if (hex_format) {
		...
		if (core->dbg->bits & RZ_SYS_BITS_64) {
			*pcstr = rz_str_newf("%" PFMT64d, frame->addr);
			*spstr = rz_str_newf("%" PFMT64d, frame->sp);
		} else if (core->dbg->bits & RZ_SYS_BITS_32) {
			*pcstr = rz_str_newf("%" PFMT64d, frame->addr);
			*spstr = rz_str_newf("%" PFMT64d, frame->sp);
		} else {
