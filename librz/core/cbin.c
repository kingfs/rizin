	...
	rz_list_foreach(list, iter, row) {
		...
		if (mode) {
			...
			if ((mode & RZ_MODE_SET)) {
				...
				rz_meta_set_string (core->analysis, RZ_META_TYPE_COMMENT, row->address, cmt);
				free (cmt);
#endif
			} else if (IS_MODE_JSON(mode)) {
				pj_a(j);
...
static void set_bin_relocs(RzCore *r, RzBinReloc *reloc, ut64 addr, Sdb **db, char **sdb_module) {
	...
	if (is_pe && !is_sandbox && reloc->import && reloc->import->name && reloc->import->libname && rz_str_startswith(reloc->import->name, "Ordinal_")) {
	...
	rz_list_foreach(sections, iter, section) {
		...
		if (IS_MODE_RAD(mode)) {
			...
			if (!section->is_segment || segments_only) {
				...
				rz_meta_set(r->analysis, RZ_META_TYPE_COMMENT, addr, 1, str);
				RZ_FREE(str);
			}
			if (section->add) {
				bool found;
	...
	do {
		...
		for (num_stringtable = 0; sdb; num_stringtable++) {
			...
			for (num_string = 0; sdb; num_string++) {
				...
				if (sdb) {
					if (!firstit_for && IS_MODE_JSON(mode)) {
						rz_cons_printf(",");
					}
					int lenkey = 0;
					int lenval = 0;
...
static bool rz_core_bin_file_print(RzCore *core, RzBinFile *bf, int mode) {
	...
	case 'j':
			...
			name ? name : "", bf->fd, bf->id, bin_sz);
		{
			...
			rz_cons_printf("{\"arch\":\"%s\",\"bits\":%d,\"binoffset\":%" PFMT64d ",\"objsize\":%" PFMT64d "}",
