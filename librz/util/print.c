...
static int rz_print_stereogram_private(const char *bump, int w, int h, char *out, int size) {
...
RZ_API char *rz_print_stereogram(const char *bump, int w, int h) {
	...
	//eprintf ("%s\n", bump);
	(void)rz_print_stereogram_private(bump, w, h, out, size);
	return out;
}
...
RZ_API char *rz_print_stereogram_bytes(const ut8 *buf, int len) {
	...
	//scr_width = rz_cons_get_size (NULL) -10;
	int cols = scr_width;
	int rows = len / cols;
	...
	bump[bumpi] = 0;
	ret = rz_print_stereogram(bump, cols, rows);
	free(bump);
	return ret;
}
