...
static int windbg_init(void) {
	...
	if (!h) {
		rz_sys_perror("LoadLibrary (\"dbgeng.dll\")");
		return 0;
	}
...
static RzIODesc *windbg_open(RzIO *io, const char *uri, int perm, int mode) {
	...
	int argc;
	char **argv = rz_str_argv(args, &argc);
	...
	int c;
	RzGetopt opt;
	rz_getopt_init(&opt, argc, (const char **)argv, "c:dgGh:i:k:op:y:z:");
	...
	case TARGET_LOCAL_SPAWN:
		if (argv[opt.ind]) {
			char *cmd = rz_str_format_msvc_argv((size_t)opt.argc - opt.ind, (const char **)argv + opt.ind);
