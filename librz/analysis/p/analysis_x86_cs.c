...
static void op0_memimmhandle(RzAnalysisOp *op, cs_insn *insn, ut64 addr, int regsz) {
	...
	case X86_OP_MEM:
		...
		if (INSOP(0).mem.base == X86_REG_RIP) {
			...
			op->stackptr = regsz;
		} else if (INSOP(0).mem.segment == X86_REG_INVALID && INSOP(0).mem.base == X86_REG_INVALID && INSOP(0).mem.index == X86_REG_INVALID && INSOP(0).mem.scale == 1) { // [<addr>]
			op->ptr = op->disp;
			if (op->ptr < 0x1000) {
				op->ptr = UT64_MAX;
			}
	...
	if (op->refptr < 1 || op->ptr == UT64_MAX) {
		...
		case X86_OP_MEM:
			...
			if (INSOP(1).mem.base == X86_REG_RIP) {
				...
				op->stackptr = regsz;
			} else if (INSOP(1).mem.segment == X86_REG_INVALID && INSOP(1).mem.base == X86_REG_INVALID && INSOP(1).mem.index == X86_REG_INVALID && INSOP(1).mem.scale == 1) { // [<addr>]
				op->ptr = op->disp;
			}
			break;
		case X86_OP_IMM:
...
static int esil_x86_cs_init(RzAnalysisEsil *esil) {
	...
	/* disable by default */
	//	rz_analysis_esil_set_interrupt (esil, 0x80, NULL);	// this is stupid, don't do this
	return true;
}
