 ...
 */
static inline ut32 rz_read_me32_arc(const void *src) {
	const ut8 *s = src;
...
static int arcompact_genops_jmp(RzAnalysisOp *op, ut64 addr, arc_fields *f, ut64 basic_type) {
	...
	case RZ_ANALYSIS_OP_TYPE_JMP:
		type_ujmp = RZ_ANALYSIS_OP_TYPE_UJMP;
		type_cjmp = RZ_ANALYSIS_OP_TYPE_CJMP;
		type_ucjmp = RZ_ANALYSIS_OP_TYPE_UCJMP;
		break;
	case RZ_ANALYSIS_OP_TYPE_CALL:
		type_ujmp = RZ_ANALYSIS_OP_TYPE_UCALL;
		type_cjmp = RZ_ANALYSIS_OP_TYPE_CCALL;
		type_ucjmp = RZ_ANALYSIS_OP_TYPE_UCCALL;
		break;
	default:
...
static int arcompact_op(RzAnalysis *analysis, RzAnalysisOp *op, ut64 addr, const ut8 *data, int len) {
	...
	case 0xf: /* General Register Format Instructions, 0x0F, [0x00 - 0x1F] */
		...
		case 0x1e:
			...
			/* RZ_ANALYSIS_OP_TYPE_SWI, but I don't know what */
			/* difference that would make to rizin */
			break;
		case 0x1f:
			op->type = RZ_ANALYSIS_OP_TYPE_TRAP;
			/* TODO: this should be RZ_ANALYSIS_OP_TYPE_DEBUG, */
